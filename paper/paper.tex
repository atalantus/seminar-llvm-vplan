...
whetever and how it is legal to vectorize the loop. This phase would produce artifacts like if 
...
These cost model artifacts would associate different vectorization possibilities with a fixed cost.
Lastly the transformation phase would vectorize the underlying IR based on all the produced 
artifacts.

However, this LoopVectorize architecture had several problems. For once extending the rigid system
to include outer loop and nested loop vectorizations would have been very challenging. Producing
and considering more and more artifacts when trying to transform the scalar into a vectorized
loop all at once does not scale well.

\subsection{State of Vectorization Plan (VPlan)}




\subsection{Future of VPlan}
%\subsection{Integration with LV}
...
\begin{figure}
  ...
  work as iteratievely applying VPlan-to-VPlan transformations.}
